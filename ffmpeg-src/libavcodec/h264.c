...
{
#if RECOMPRESS
    const int big_mb_num = h->mb_stride * (h->mb_height + 1) * 2;//double 内存分配
#else
...
#if RECOMPRESS
    const int offset = big_mb_num / 2;//乘2后除2保持offset不变
#endif
...
#if RECOMPRESS
            h->mb2b_xy[mb_xy + offset]  = h->mb2b_xy[mb_xy]; //保存量化前后系数
            h->mb2br_xy[mb_xy + offset] = h->mb2br_xy[mb_xy];
#endif
        }
...
#if RECOMPRESS
    h->slice_ctx = av_mallocz_array(2 * h->nb_slice_ctx, sizeof(*h->slice_ctx));//double
#else
...
#if RECOMPRESS
            if(h->avctx->speedup){//开启加速，跳过pb
                if(h->pict_type == AV_PICTURE_TYPE_B || h->pict_type == AV_PICTURE_TYPE_P){
                    ret = 0;
                    goto end;//跳过解码的过程，直接传输原码流
                }
            }
...
#if RECOMPRESS
    FILE *reloadFile;
    if(h->avctx->compressMode == 1){//重压缩
        reloadFile = h->avctx->reCompressStreamFile;
    }
    else if(h->avctx->compressMode == 2){ //解码恢复
        reloadFile = h->avctx->deCompressStreamFile;
    }
    if(reloadFile){
        if(h->cur_pic_ptr->tf.f->pict_type != AV_PICTURE_TYPE_I){
            fwrite(buf, buf_size, 1, reloadFile);//这里写错过
        }
        else{//先只处理I帧
            uint8_t *ptr = buf;
            int s_ctx = 0;
            for(int i = 0; i < h->pkt.nb_nals; ++i){
                H2645NAL *nal = &h->pkt.nals[i];
                if(nal->type == NAL_IDR_SLICE || nal->type == NAL_SLICE){
                    //output slice header
                    int full_len = 0;
                    int ssz = get_bits_count(&(h->slice_ctx[s_ctx].gb)) / 8;
                    int len_start_code = (nal->raw_data - ptr);
                    int len_header = ssz + len_start_code;
                    PutBitContext *pb = &(h->slice_ctx[s_ctx].cabac_enc.pb);//保存重编后码流
                    int len_data = (pb->buf_ptr - pb->buf);
                    full_len = len_header + len_data;

                    //temp buffer to insert emulation prevention bytes
                    uint8_t *b_final = (uint8_t *)av_malloc(full_len << 2);
                    assert(b_final != NULL);
                    memcpy(b_final, ptr, len_start_code);
                    memcpy(b_final + len_start_code, h->slice_ctx[s_ctx].gb.buffer, ssz);
                    memcpy(b_final + len_header, pb->buf, len_data);

                    //emultaion prevention bytes
                    int first_start_code = 0;
                    uint8_t *b = b_final;
                    uint8_t *b_end = b_final + full_len;
                    uint8_t *b_out = b_final + (full_len << 1);
                    uint8_t *b_out_start = b_out;
                    ...
                    *b_out++ = *b++;
                    *b_out++ = *b++;//first two bytes
                    int b3count = 0;
                    for(; b != b_end; b++){
                        //skip first two bytes
                        if(!b_out[-2] && !b_out[-1] && (*b) <= 3){
                            *b_out++ = 0x3;
                            b3count++;
                        }
                        *b_out++ = *b;
                    ...
                    //check last two bytes as nal should not end with 0 byte
                    if(!b_out[-1] && !b_out[-2]){
                        *b_out++ = 0x3;
                    }

                    //temp start code
                    int l = b_out - b_out_start;
                    fwrite(b_out_start, l,1, reloadFile);
                    ptr += nal->raw_size + (nal->raw_data - ptr) - 1;//起始码长度
                    s_ctx ++;
                    free(b_final);
                    //need to free cabac write buffer
                    free(pb->buf);
                    pb->buf = NULL;
                    pb->bit_buf = 0;
                }else{//I帧PPS SPS SEI等，直接重写回文件
                    int len = nal->raw_size + (nal->raw_data - ptr) - 1;
                    fwrite(ptr, len, 1, reloadFile);
                    ptr += len;
                }
            }
        }
