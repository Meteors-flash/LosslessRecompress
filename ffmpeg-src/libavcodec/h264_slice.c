...
static int h264_slice_header_init(H264Context *h)
{
    ...
    if (!HAVE_THREADS || !(h->avctx->active_thread_type & FF_THREAD_SLICE)) {
        ...
        if(h->pict_type == AV_PICTURE_TYPE_I){
            h->slice_ctx[0].enc_cache.t_offset = h->mb_stride * (h->mb_height + 1);
            h->slice_ctx[0].enc_cache.intra4x4_pred_mode = h->intra4x4_pred_mode 
                + i * 8 * 2 * h->mb_stride + h->slice_ctx[0].enc_cache.t_offset;
        }
#endif
    } else {
...
#if RECOMPRESS
            if(h->pict_type == AV_PICTURE_TYPE_I){
            sl->enc_cache.t_offset = h->mb_stride * (h->mb_height + 1);
            sl->enc_cache.intra4x4_pred_mode = h->intra4x4_pred_mode 
                + i * 8 * 2 * h->mb_stride + sl->enc_cache.t_offset;
        }
#endif
...
static int decode_slice(struct AVCodecContext *avctx, void *arg)
{
    ...
    if (h->pps.cabac) {
        ...
        if (ret < 0)
            return ret;
#if RECOMPRESS
        sl->enc_cache.qscale = sl->qscale;
        sl->enc_cache.last_qscale_diff = 0;

        int sz = get_bits_left(&sl->gb) / 8;//根据输入码流大小确定重压缩码流大小
        sz *= 100;
        uint8_t *ptr = (uint8_t *)malloc(sz);
        init_put_bits(&sl->pb, ptr, sz);//为重压缩码流分配内存空间
        ff_init_cabac_encoder(&sl->cabac_enc, sl->pb.buf_ptr + put_bits_count(&sl->pb) / 8, (put_bits_left(&sl->pb) + 7) / 8);

        ff_h264_init_cabac_states(h, sl);//初始化cabac状态
#else
        ff_h264_init_cabac_states(h, sl);//初始化cabac状态
#endif
        for (;;) {
...
#if RECOMPRESS
            if(sl->slice_type_nos == AV_PICTURE_TYPE_I && !eos){//eos:0 terminal; 1 terminal + flush
...
#if RECOMPRESS
                if(sl->slice_type_nos == AV_PICTURE_TYPE_I){
                    cabac_encode_flush(&sl->cabac_enc, 0);//flush到内存中去
                }
#endif
...
int ff_h264_execute_decode_slices(H264Context *h, unsigned context_count)
{
    ...
    av_assert0(context_count && h->slice_ctx[context_count - 1].mb_y < h->mb_height);//mb未超过范围
