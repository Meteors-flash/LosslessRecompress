...
#if RECOMPRESS
    DECLARE_ALIGNED(16, int16_t, mb)[16 * 48 * 2 * 2];//double space
    DECLARE_ALIGNED(16, int16_t, mb_luma_dc)[3][16 * 2 * 2];
#else
...
#if RECOMPRESS
    //encode cabac
    CABACContext cabac_enc;
    uint8_t cabac_state_enc[1024];
#endif
...
static av_always_inline int pred_intra_mode_enc(const H264Context *h,
                                        H264SliceContext *sl, int n)
{
    const int index8 = scan8[n];
    const int left   = sl->enc_cache.intra4x4_pred_mode_cache[index8 - 1];
    const int top    = sl->enc_cache.intra4x4_pred_mode_cache[index8 - 8];
    const int min    = FFMIN(left, top);
