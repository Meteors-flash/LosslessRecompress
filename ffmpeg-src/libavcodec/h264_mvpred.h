...
#if RECOMPRESS
static void fill_encode_neighbors(const H264Context *h, H264SliceContext *sl, int mb_type)
{
    const int mb_xy = sl->mb_xy;
    int topleft_xy, top_xy, topright_xy, left_xy[LEFT_MBS];
    static const uint8_t left_block_options[4][32] = {
    ...
    sl->enc_cache.topleft_partition = -1;
    ...
    if (FRAME_MBAFF(h)) {
        ...
        if (sl->mb_y & 1) {
            if (left_mb_field_flag != curr_mb_field_flag) {
                ...
                if (curr_mb_field_flag) {
                    left_xy[LBOT] += h->mb_stride;
                    sl->enc_cache.left_block  = left_block_options[3];
                } else {
                    topleft_xy += h->mb_stride;
                    /* take top left mv from the middle of the mb, as opposed
                     * to all other modes which use the bottom right partition */
                    sl->enc_cache.topleft_partition = 0;
                    sl->enc_cache.left_block        = left_block_options[1];
                }
            }
        } else {
            if (curr_mb_field_flag) {
            ...
            if (left_mb_field_flag != curr_mb_field_flag) {
                if (curr_mb_field_flag) {
                    ...
                    sl->enc_cache.left_block = left_block_options[2];
                }
            }
        }
    }

    sl->enc_cache.topleft_mb_xy    = topleft_xy;
    sl->enc_cache.top_mb_xy        = top_xy;
    sl->enc_cache.topright_mb_xy   = topright_xy;
    sl->enc_cache.left_mb_xy[LTOP] = left_xy[LTOP];
    sl->enc_cache.left_mb_xy[LBOT] = left_xy[LBOT];
    //FIXME do we need all in the context?

    sl->enc_cache.topleft_type    = h->cur_pic.mb_type[topleft_xy];
    sl->enc_cache.top_type        = h->cur_pic.mb_type[top_xy];
    sl->enc_cache.topright_type   = h->cur_pic.mb_type[topright_xy];
    sl->enc_cache.left_type[LTOP] = h->cur_pic.mb_type[left_xy[LTOP]];
    sl->enc_cache.left_type[LBOT] = h->cur_pic.mb_type[left_xy[LBOT]];

    if (FMO) {
        if (h->slice_table[topleft_xy] != sl->slice_num)
            sl->enc_cache.topleft_type = 0;
        if (h->slice_table[top_xy] != sl->slice_num)
            sl->enc_cache.top_type = 0;
        if (h->slice_table[left_xy[LTOP]] != sl->slice_num)
            sl->enc_cache.left_type[LTOP] = sl->left_type[LBOT] = 0;
    } else {
        if (h->slice_table[topleft_xy] != sl->slice_num) {
            sl->enc_cache.topleft_type = 0;
            if (h->slice_table[top_xy] != sl->slice_num)
                sl->enc_cache.top_type = 0;
            if (h->slice_table[left_xy[LTOP]] != sl->slice_num)
                sl->enc_cache.left_type[LTOP] = sl->enc_cache.left_type[LBOT] = 0;
        }
    }
    if (h->slice_table[topright_xy] != sl->slice_num)
        sl->enc_cache.topright_type = 0;
}

static void fill_encode_caches(const H264Context *h, H264SliceContext *sl, int mb_type)
{
    int topleft_xy, top_xy, topright_xy, left_xy[LEFT_MBS];
    int topleft_type, top_type, topright_type, left_type[LEFT_MBS];
    const uint8_t *left_block = sl->left_block;
    ...
    topleft_xy      = sl->enc_cache.topleft_mb_xy;
    top_xy          = sl->enc_cache.top_mb_xy;
    topright_xy     = sl->enc_cache.topright_mb_xy;
    left_xy[LTOP]   = sl->enc_cache.left_mb_xy[LTOP];
    left_xy[LBOT]   = sl->enc_cache.left_mb_xy[LBOT];
    topleft_type    = sl->enc_cache.topleft_type;
    top_type        = sl->enc_cache.top_type;
    topright_type   = sl->enc_cache.topright_type;
    left_type[LTOP] = sl->enc_cache.left_type[LTOP];
    left_type[LBOT] = sl->enc_cache.left_type[LBOT];

    if (!IS_SKIP(mb_type)) {
        if (IS_INTRA(mb_type)) {
            int type_mask = h->pps.constrained_intra_pred ? IS_INTRA(-1) : -1;
            sl->enc_cache.topleft_samples_available     =
                sl->enc_cache.top_samples_available     =
                    sl->enc_cache.left_samples_available = 0xFFFF;
            sl->enc_cache.topright_samples_available     = 0xEEEA;

            if (!(top_type & type_mask)) {
                sl->enc_cache.topleft_samples_available  = 0xB3FF;
                sl->enc_cache.top_samples_available      = 0x33FF;
                sl->enc_cache.topright_samples_available = 0x26EA;
            }
            if (IS_INTERLACED(mb_type) != IS_INTERLACED(left_type[LTOP])) {
                if (IS_INTERLACED(mb_type)) {
                    if (!(left_type[LTOP] & type_mask)) {
                        sl->enc_cache.topleft_samples_available &= 0xDFFF;
                        sl->enc_cache.left_samples_available    &= 0x5FFF;
                    }
                    if (!(left_type[LBOT] & type_mask)) {
                        sl->enc_cache.topleft_samples_available &= 0xFF5F;
                        sl->enc_cache.left_samples_available    &= 0xFF5F;
                    }
                } else {
                    int left_typei = h->cur_pic.mb_type[left_xy[LTOP] + h->mb_stride];
                    ...
                    av_assert2(left_xy[LTOP] == left_xy[LBOT]);
                    if (!((left_typei & type_mask) && (left_type[LTOP] & type_mask))) {
                        sl->enc_cache.topleft_samples_available &= 0xDF5F;
                        sl->enc_cache.left_samples_available    &= 0x5F5F;
                    }
                }
            } else {
                if (!(left_type[LTOP] & type_mask)) {
            ...
            if (!(topleft_type & type_mask))
                sl->enc_cache.topleft_samples_available &= 0x7FFF;

            if (!(topright_type & type_mask))
                sl->enc_cache.topright_samples_available &= 0xFBFF;

            if (IS_INTRA4x4(mb_type)) {
                if (IS_INTRA4x4(top_type)) {
                    AV_COPY32(sl->enc_cache.intra4x4_pred_mode_cache + 4 + 8 * 0, sl->enc_cache.intra4x4_pred_mode + h->mb2br_xy[top_xy]);
                } else {
                    sl->enc_cache.intra4x4_pred_mode_cache[4 + 8 * 0] =
                    sl->enc_cache.intra4x4_pred_mode_cache[5 + 8 * 0] =
                    sl->enc_cache.intra4x4_pred_mode_cache[6 + 8 * 0] =
                    sl->enc_cache.intra4x4_pred_mode_cache[7 + 8 * 0] = 2 - 3 * !(top_type & type_mask);
                }
                for (i = 0; i < 2; i++) {
                    if (IS_INTRA4x4(left_type[LEFT(i)])) {
                        int8_t *mode = sl->intra4x4_pred_mode + h->mb2br_xy[left_xy[LEFT(i)]];
                        sl->enc_cache.intra4x4_pred_mode_cache[3 + 8 * 1 + 2 * 8 * i] = mode[6 - left_block[0 + 2 * i]];
                        sl->enc_cache.intra4x4_pred_mode_cache[3 + 8 * 2 + 2 * 8 * i] = mode[6 - left_block[1 + 2 * i]];
                    } else {
                        sl->enc_cache.intra4x4_pred_mode_cache[3 + 8 * 1 + 2 * 8 * i] =
                        sl->enc_cache.intra4x4_pred_mode_cache[3 + 8 * 2 + 2 * 8 * i] = 2 - 3 * !(left_type[LEFT(i)] & type_mask);
                    }
                }
            }
        }
        ...
        /* FIXME: constraint_intra_pred & partitioning & nnz
         * (let us hope this is just a typo in the spec) */
        nnz_cache = sl->enc_cache.non_zero_count_cache;
        if (top_type) {
            nnz = h->non_zero_count[top_xy + sl->enc_cache.t_offset];//diff
            AV_COPY32(&nnz_cache[4 + 8 * 0], &nnz[4 * 3]);
            if (!h->chroma_y_shift) {
                AV_COPY32(&nnz_cache[4 + 8 *  5], &nnz[4 *  7]);
                AV_COPY32(&nnz_cache[4 + 8 * 10], &nnz[4 * 11]);
        ...
        for (i = 0; i < 2; i++) {
            if (left_type[LEFT(i)]) {
                nnz = h->non_zero_count[left_xy[LEFT(i)] + sl->enc_cache.t_offset];//diff
                nnz_cache[3 + 8 * 1 + 2 * 8 * i] = nnz[left_block[8 + 0 + 2 * i]];
                nnz_cache[3 + 8 * 2 + 2 * 8 * i] = nnz[left_block[8 + 1 + 2 * i]];
                if (CHROMA444(h)) {
                    nnz_cache[3 + 8 *  6 + 2 * 8 * i] = nnz[left_block[8 + 0 + 2 * i] + 4 * 4];
        ...
        if (CABAC(h)) {
            // top_cbp
            if (top_type)
                sl->enc_cache.top_cbp = h->cbp_table[top_xy];
            else
                sl->enc_cache.top_cbp = IS_INTRA(mb_type) ? 0x7CF : 0x00F;
            // left_cbp
            if (left_type[LTOP]) {//diff: + t_offset
                sl->enc_cache.left_cbp =   (h->cbp_table[left_xy[LTOP] + sl->enc_cache.t_offset] & 0x7F0) |
                               ((h->cbp_table[left_xy[LTOP] + sl->enc_cache.t_offset] >> (left_block[0] & (~1))) & 2) |
                              (((h->cbp_table[left_xy[LBOT] + sl->enc_cache.t_offset] >> (left_block[2] & (~1))) & 2) << 2);
            } else {
                sl->enc_cache.left_cbp = IS_INTRA(mb_type) ? 0x7CF : 0x00F;
            }
        }
    }

    if (IS_INTER(mb_type) || (IS_DIRECT(mb_type) && sl->enc_cache.direct_spatial_mv_pred)) {
        int list;
        int b_stride = h->b_stride;
        for (list = 0; list < sl->list_count; list++) {
            int8_t *ref_cache = &sl->ref_cache[list][scan8[0]];//add
            int8_t *ref = h->cur_pic.ref_index[list];//add
            int16_t(*mv_cache)[2] = &sl->mv_cache[list][scan8[0]];
            int16_t(*mv)[2]       = h->cur_pic.motion_val[list];
            if (!USES_LIST(mb_type, list))
                continue;
            av_assert2(!(IS_DIRECT(mb_type) && !sl->enc_cache.direct_spatial_mv_pred));
            ...
            if(ref_cache[2 - 1*8] < 0 || ref_cache[4 - 1 * 8] < 0) {
                if (USES_LIST(topleft_type, list)) {
                    const int b_xy  = h->mb2b_xy[topleft_xy] + 3 + b_stride +
                                      (sl->topleft_partition & 2 * b_stride);//这里沿用原来的不用enc_cache.
                    const int b8_xy = 4 * topleft_xy + 1 + (sl->topleft_partition & 2);
                    AV_COPY32(mv_cache[-1 - 1 * 8], mv[b_xy]);
                    ref_cache[-1 - 1 * 8] = ref[b8_xy];
                } else {
...
#undef MAP_F2F
    ...
    sl->enc_cache.neighbor_transform_size = !!IS_8x8DCT(top_type) + !!IS_8x8DCT(left_type[LTOP]);
}
#endif
/**
