...
static void
h264_video_decode(const char *filename, const char *reCompressStreamName, const char *deCompressStreamName)//加入重压缩流和解码流
#else
...
#endif
{
    ...
    if (reCompressStreamName != NULL) {
        ...
        codec_ctx->deCompressStreamFile = fopen(deCompressStreamName, "wb");
...
//打印结果
static void
printInfo(struct timeval start, struct timeval end, const char *in_file, const char *recon_stream, const char *cmd) {
    ...
    printf("inize-%llu, outSize-%llu, ratio-%.4f, speed-%.2f MB/s\n", inSize, reconSize, fRatio, fSpeed);
}
